...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calculate total (using provided item price from cart, fallback to product base price)
    const products = await getProducts();
    let totalAmount = 0;
    const processedItems: { productId: string; variantId?: string; quantity: number; price: number; name?: string; variantName?: string }[] = [];
    ...
    for (const item of items) {
      ...
      // Resolve variant name from payload first, and use product's variant data as fallback.
      const selectedVariant = item.variantId
        ? product.variants?.find((v) => v.id === item.variantId)
        : undefined;
      const resolvedVariantName =
        (item.variantName && item.variantName.toString().trim()) ||
        selectedVariant?.name ||
        undefined;

      // Diagnostic logging
      console.log(`[ORDER] Processing item: ${product.name}`, {
        variantId: item.variantId,
        payloadVariantName: item.variantName,
      ...
      processedItems.push({
        ...
        price: unitPrice,
        name: typeof item.name === 'string' && item.name ? item.name : product.name,
