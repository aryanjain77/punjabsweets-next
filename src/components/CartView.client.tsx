...
export function CartView() {
  ...
  const [qrCodeUrl, setQrCodeUrl] = useState<string>(SHOP_CONFIG.qrCodeImageUrl);
  ...
  const cartItems = items
    .map((item) => {
      ...
      // Ensure variantName is captured from variant if not in cart
      ...
      if (item.variantId) {
        if (!variant?.name && !item.variantName) {
          console.error(`⚠️ MISSING VARIANT INFO for ${product?.name} (variantId: ${item.variantId}, item.variantName: "${item.variantName}")`);
        } else {
          console.log(`✓ Variant captured for ${product?.name}: "${resolvedVariantName}"`);
        }
      }
  ...
  useEffect(() => {
    ...
    if (lastPaymentMethod !== "upi") return;
    ...
    fetch("/api/qr-code")
      ...
      .then((data) => {
        ...
        if (!cancelled && typeof url === "string" && url) setQrCodeUrl(url);
      })
      .catch(() => {
        // keep fallback from SHOP_CONFIG
      });

    return () => {
      cancelled = true;
    };
  }, [lastPaymentMethod, successOrderId]);
      ...
      <div className="rounded-2xl bg-gradient-to-br from-green-50/30 to-emerald-50/30 backdrop-blur-xl p-8 text-center ring-1 ring-green-200/50 border border-green-300/30 shadow-xl">
        ...
        <button
          onClick={() => {
            ...
            setQrCodeUrl(SHOP_CONFIG.qrCodeImageUrl);
            clearCart();
          }}
          className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-amber-500 to-rose-500 px-6 py-3 text-sm font-semibold text-white hover:shadow-lg transition-all"
        >
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CRITICAL: Fetch fresh product data with variants before submission
      // This ensures we have the latest variant info
      let freshProducts = products;
      try {
        console.log('[CartView] Fetching fresh product data before order submission...');
        
        const freshRes = await fetch('/api/products');
        if (freshRes.ok) {
          const freshData = await freshRes.json();
          freshProducts = freshData.products ?? products;
          console.log('[CartView] Fresh products loaded:', freshProducts.length);
        }
      } catch (err) {
        console.warn('[CartView] Could not refresh products, using cached data:', err);
      }
      ...
      // Re-enrich cart items with fresh product data to ensure variant names
      const enrichedOrderItems = cartItems.map((item) => {
        ...
        // Use variant name from fresh data, fallback to cached, then undefined
        const variantName = item.variantName ?? freshVariant?.name;

        const orderItem = {
          productId: item.productId,
          variantId: item.variantId,
          quantity: item.quantity,
          price: item.variant?.price ?? freshProduct?.price ?? item.product?.price ?? 0,
          name: freshProduct?.name ?? item.product?.name ?? '',
          variantName: variantName || undefined,
        };

        // Log for debugging
        if (orderItem.variantId) {
          console.log(`[Order] Item: ${orderItem.name}`, {
            variantId: orderItem.variantId,
            sentvariantName: variantName,
            sources: {
              fromCart: item.variantName,
              fromFreshVariant: freshVariant?.name,
              fromCachedVariant: item.variant?.name,
            },
          });
        }
      ...
      console.log("ORDER ITEMS:", enrichedOrderItems);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white/30 backdrop-blur-xl ring-1 ring-white/20 border border-white/40 shadow-xl overflow-hidden">
        ...
        <div className="divide-y divide-white/20">
          {cartItems.map((item) => (
            <div key={`${item.productId}-${item.variant?.id ?? 'default'}`} className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 py-3 sm:px-6 sm:py-4 hover:bg-white/10 transition">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <p className="flex-1 sm:w-16 text-right text-sm font-bold text-amber-600">
                  ₹{((item.variant?.price ?? item.product?.price ?? 0) * item.quantity).toFixed(0)}
                </p>
                <div className="flex items-center gap-1 bg-gray-200/50 rounded-lg p-1">
                  <button
                    onClick={() => setQuantity(item.productId, Math.max(0, item.quantity - 1), item.variant?.id)}
                  ...
                  <button
                    onClick={() => setQuantity(item.productId, item.quantity + 1, item.variant?.id)}
                    className="rounded-md bg-gray-300/50 hover:bg-gray-400/50 px-2 py-1 text-sm font-semibold transition"
                  >
                    +
                  </button>
